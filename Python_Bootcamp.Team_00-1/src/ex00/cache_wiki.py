...
class WikiParser:
    ...
    def parse_wiki_page(self, link):
        ...
        links = [link for link in links if not link['href'].lower().endswith(('.svg', '.png', '.jpg', '.jpeg'))]
        for link in links:
...
def parse_arguments():
    ...
    if args.max_depth is None: args.max_depth = 3
    if args.article_link is None : args.article_link = DEFAULT
    else:
